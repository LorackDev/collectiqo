...
Neben den Ports 80 und 443 (Übertragungsprotokolle http und https), die für den NGINX-Reverse-Proxy genutzt werden, waren auch die Ports der Datenbank-Container (MySQL und MongoDB) offen erreichbar.
...
\vspace{1em}
\begin{lstlisting}[label={lst:lst-dockercompose-networks}, caption=Netzwerke in der Docker-Compose-Datei]
networks:
  public-network:
    driver: bridge
  private-network:
...
Der Befehlsparameter \textit{--force-renewal} sorgt dafür, dass die Zertifikate auch dann erneuert werden, wenn sie noch länger als 30 Tage gültig sind.
...
Die Konfiguration des NGINX-Reverse-Proxys wurde entsprechend in der \textit{nginx.conf} angepasst, um Anfragen über IPv6 zu unterstützen.
...
\vspace{1em}
\begin{lstlisting}[label={lst:lst-nginx-config}, caption=NGINX-Konfiguration für IPv4 und IPv6]
    server {
...
Weitreichendste Änderung war die Implementierung von Healthchecks für die Docker-Container.
...
\vspace{1em}
\begin{lstlisting}[label={lst:lst-dockercompose-container-healthchecks}, caption=Healthchecks in der Docker-Compose-Datei]
services:
  mysql:
...
Für den \textbf{NGINX} Container wird per curl erprobt, ob der HTTP-Endpunkt erreichbar ist (http://localhost/health).
...
Container, die von anderen Containern abhängig sind, werden also erst gestartet, wenn die abhängigen Container als „healthy“ markiert sind.
...
\vspace{1em}
\begin{lstlisting}[label={lst:lst-systemd-service}, caption=Systemd-Service-Datei für Docker-Compose]
[Unit]
