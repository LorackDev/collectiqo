...
\subsection{Frontend}\label{subsec:nutzereinstellungen-frontend}
Im Frontend lag der Fokus auf der optischen und funktionalen Überarbeitung der bereits vorhandenen Eingabefelder.
Diese wurden zentriert und visuell besser in die Seite eingebettet, um eine klarere Struktur und ein ansprechenderes Gesamtbild zu schaffen.
Ergänzt wurde zudem eine einfache Animation beim Wechsel zwischen den einzelnen Bearbeitungsbereichen, um die Nutzerführung intuitiver zu gestalten.
Das überarbeitete Layout orientiert sich am bestehenden Design und sorgt für eine konsistente Benutzererfahrung innerhalb der Anwendung.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{accountsettings_new}
    \caption{Überarbeitung der Account Settings}
    \label{fig:accountsettings-2}
\end{figure}

\subsection{Backend}\label{subsec:nutzereinstellungen-backend}
