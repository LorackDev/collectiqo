...
\subsection{Frontend}\label{subsec:sammlungsansicht-frontend}
Im Frontend wurde die Sammlungsansicht um eine dynamisch bearbeitbare Tabellenkomponente erweitert.
Nutzer haben nun die Möglichkeit, direkt innerhalb der Tabelle neue Zeilen hinzuzufügen, bestehende Inhalte zu verändern oder Einträge zu entfernen.
Dabei erfolgt die Interaktion vollständig clientseitig, wodurch Änderungen unmittelbar sichtbar sind, ohne die Seite neu laden zu müssen.
Die Eingaben werden in Echtzeit im Browser verarbeitet und können anschließend per Klick auf den „Save“-Button an den Server übermittelt werden.
Die visuelle Gestaltung der Tabelle orientiert sich am bestehenden Design und wurde durch gezielte Abstände und Hover-Effekte benutzerfreundlicher gestaltet.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{tabelle_dyn_1}
    \caption{Anzeigemodus der Tabelle}
    \label{fig:tabelle-dyn-1}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{tabelle_dyn_2}
    \caption{Bearbeitungsmodus der Tabelle}
    \label{fig:tabelle-dyn-2}
\end{figure}

\subsection{Backend}\label{subsec:sammlungsansicht-backend}
